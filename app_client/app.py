...
_GuardedDB bloque tout DELETE non scope sur ces tables.
...
from flask import Flask, render_template, request, redirect, url_for, jsonify, g, make_response, session, send_file
...
def init_db():
    ...
    # (Le seed par-restaurant des absences est désormais géré par _ensure_default_absences appelé à chaque visite)
...
def analyse_depenses_data():
    ...
    if periode == "mois":
        ...
        # Dernier jour du mois precedent = 1er du mois courant - 1 jour
        prev_to = (__import__("datetime").date(today.year, today.month, 1) - timedelta(days=1)).isoformat()
    elif periode == "3mois":
    ...
    d_iter = __import__("datetime").date.fromisoformat(date_from) if date_from > "2000-01-01" else __import__("datetime").date(today.year, today.month, 1)
    while d_iter <= today:
    ...
    # --- PAR FOURNISSEUR (uniquement ceux de la table fournisseurs) ---
    # Utilise la colonne mois (comme _depenses_mois) pour cohérence avec les totaux catégorie
    ...
    # Recurring fixes for total calculation only (not shown as fournisseurs)
    ...
    # fixes_as_fourns kept for total calculation but not shown in fournisseurs list
    all_fourns.sort(key=lambda x: -x["total"])
    for f in all_fourns:
        f["pct"] = round(f["total"] / total_var * 100, 1) if total_var > 0 else 0
    fournisseurs = all_fourns

    # Repartition par categorie (for pie chart)
    type_totals = {}
    for f in fournisseurs:
        t = f["type"]
    ...
    # --- PAR CATEGORIE : utilise _depenses_mois() par mois pour coherence ---
    total_all = 0
    total_variables = 0
    total_fixes = 0
    agg_food = 0
    ...
    # CA de la periode — utilise les mêmes mois que _depenses_mois pour cohérence
    ca_periode = 0
    ...
    # Resultat net coherent avec la page ratios
    resultat_net = round((ca_periode - total_all) / ca_periode * 100, 1) if ca_periode > 0 else 0

    # Periode precedente pour evolution
    prev_mois_list = set()
    if prev_from and prev_to:
        try:
            d_iter2 = __import__("datetime").date.fromisoformat(prev_from)
            prev_end = __import__("datetime").date.fromisoformat(prev_to)
...
def depenses():
    ...
    # Recurring fixes from previous months that apply to this month
    ...
    # Build fixes list: direct + recurring (avoid duplicates by categorie+description)
    ...
    for r in recurrentes:
        ...
        if key not in direct_keys:
            # Check if disabled for this month
    ...
    # Données récap — calculées depuis fixes_list et variables (source unique de vérité)
...
def depenses_scan():
    ...
    # Normaliser les types non supportes par Claude Vision (HEIC iOS, etc.)
    supported_types = {"image/jpeg", "image/png", "image/webp", "image/gif"}
    if content_type not in supported_types:
        print(f"[SCAN] Content-type non supporte '{content_type}', force image/jpeg")
    ...
    if r2_ok():
        try:
            ...
            photo_key = upload_file(fobj, folder, fname)
            photo_url = get_file_url(photo_key, expires=3600)
        except Exception as e:
    ...
    import requests as _req
    prompt = (
        "Tu es un assistant specialise dans l'extraction de donnees de factures francaises. "
        "Analyse cette image de facture et retourne UNIQUEMENT un JSON valide sans aucun texte autour, "
        "avec ces champs exactement : {\"fournisseur\": \"\", \"numero_facture\": \"\", \"date_facture\": \"\", "
        "\"date_echeance\": \"\", \"montant_ht\": 0, \"montant_tva\": 0, \"montant_ttc\": 0, "
        "\"produits\": [{\"description\": \"\", \"quantite\": 0, \"prix_unitaire\": 0, \"montant\": 0}], "
        "\"notes\": \"\"}. Si un champ est illisible ou absent mets null. Les montants sont en euros, "
        "nombres seulement sans symbole. La date en format dd/mm/yyyy."
    )
    facture = None
    api_error = ""
    try:
        resp = _req.post("https://api.anthropic.com/v1/messages", headers={
            "x-api-key": api_key,
        ...
        if resp.status_code != 200:
            api_error = f"API Anthropic erreur {resp.status_code}"
            print(f"[SCAN] API error {resp.status_code}: {resp.text[:500]}")
        else:
            resp_data = resp.json()
            ...
            if "```" in text:
                ...
                if text.startswith("json"):
                    text = text[4:]
            facture = _json.loads(text.strip())
            print(f"[SCAN] Claude Vision OK: fournisseur={facture.get('fournisseur')}, ttc={facture.get('montant_ttc')}")
    except _req.exceptions.Timeout:
        api_error = "D\u00e9lai d\u00e9pass\u00e9 — r\u00e9essayez avec une image plus nette."
        print("[SCAN] Claude Vision timeout")
    except Exception as e:
        api_error = f"Erreur analyse : {e}"
        print(f"[SCAN] Claude Vision error: {e}")

    if not facture:
        return jsonify({"ok": False, "error": api_error or "Impossible de lire la facture.",
                        "error_type": "vision", "photo_key": photo_key, "photo_url": photo_url}), 200

    # Fuzzy match fournisseur avec la liste existante
    db = get_db()
    fournisseurs_db = db.execute("SELECT id, nom, type FROM fournisseurs WHERE restaurant_id=? ORDER BY nom", (_rid(),)).fetchall()
    fournisseurs_list = [{"id": f["id"], "nom": f["nom"], "type": f["type"]} for f in fournisseurs_db]
    fourn_match = ""
    fourn_name = (facture.get("fournisseur") or "").strip().lower()
    if fourn_name:
        # 1) Exact match
        for f in fournisseurs_list:
            if f["nom"].lower() == fourn_name:
                fourn_match = f["nom"]
                break
        ...
        if not fourn_match:
            for f in fournisseurs_list:
                fn = f["nom"].lower()
                if fn in fourn_name or fourn_name in fn:
                    fourn_match = f["nom"]
                    break
        # 3) First-word fuzzy match (e.g. "METRO CASH & CARRY" → "Metro")
        if not fourn_match:
            first_word = fourn_name.split()[0] if fourn_name.split() else ""
            if first_word and len(first_word) >= 3:
                for f in fournisseurs_list:
                    if f["nom"].lower().startswith(first_word):
                        fourn_match = f["nom"]
                        break
...
def depenses_scan_save():
    """Sauvegarde une facture scannee dans SQLite + Airtable."""
    data = request.get_json(silent=True) or {}
    db = get_db()
...
def depenses_fournisseur_quick():
    """Creation rapide d'un fournisseur depuis le scan facture."""
    data = request.get_json(silent=True) or {}
    nom = data.get("nom", "").strip()
    ftype = data.get("type", "Autre")
    if not nom:
...
def depenses_export():
    ...
    # Construit la liste des mois cibles couverts par la période (YYYY-MM)
    ...
    # Variables : déjà attachées à un mois précis
    ...
    # Fixes : pour chaque dépense fixe, l'étendre sur tous les mois cibles où elle est active
    # Active si : mois_creation <= mois_cible ET (recurrente=1 OU mois_creation == mois_cible)
    # ET le mois cible n'est pas dans desactivee_mois
    ...
    # Normalisation des dates au format DD/MM/YYYY pour TOUS les exports
    def _fmt_date(s):
        s = (s or "").strip()
        if not s:
            return ""
        ...
        # Format YYYY-MM (dépense fixe sans jour) → 1er du mois
        if len(s) == 7 and s[4] == "-":
            return f"01/{s[5:7]}/{s[:4]}"
        return s
    for r in rows:
    ...
    if fmt == "csv":
        ...
        for r in rows:
            writer.writerow([r["date_fmt"], r["categorie"], r["fournisseur"], r["montant"], r["type"]])
        wrapper.flush()
        wrapper.detach()
        resp = make_response(output.getvalue())
        ...
        ws = wb.active
        ws.title = "Depenses"
        ws.append(["Date", "Categorie", "Fournisseur", "Montant", "Type"])
        for r in rows:
            ws.append([r["date_fmt"], r["categorie"], r["fournisseur"], r["montant"], r["type"]])
        ws.append(["TOTAL", "", "", total_montant, ""])
        # Auto-fit largeur des colonnes (min 12, max 50)
        for col_idx, col_cells in enumerate(ws.columns, 1):
            max_length = max((len(str(cell.value)) if cell.value is not None else 0) for cell in col_cells)
            ws.column_dimensions[get_column_letter(col_idx)].width = max(12, min(max_length + 4, 50))
        buf = io.BytesIO()
        wb.save(buf)
        buf.seek(0)
        resp = make_response(buf.getvalue())
        ...
        resto_name = (current_user.restaurant_name or "Mon Restaurant") if current_user.is_authenticated else ""
        periode_label = {"mois": "Mois en cours", "trimestre": "Trimestre", "annee": "Année", "tout": "Toutes périodes"}.get(periode, periode)

        buf = io.BytesIO()
        doc = SimpleDocTemplate(buf, pagesize=A4,
                                leftMargin=2*cm, rightMargin=2*cm,
                                topMargin=2*cm, bottomMargin=2*cm,
                                title=f"Dépenses {periode_label} — {resto_name}")
        styles = getSampleStyleSheet()
        ...
        # Header : table 2 colonnes (logo gauche / meta droite)
        meta_html = f"<b>{resto_name}</b><br/>Dépenses — {periode_label}"
        header_tbl = Table(
            [[Paragraph("PAUCO", logo_style), Paragraph(meta_html, meta_style)]],
            colWidths=[8*cm, 9*cm]
        )
        ...
        # Tableau dépenses
        cell_style = ParagraphStyle("cell", parent=styles["Normal"], fontName="Helvetica", fontSize=8, leading=11)
        cell_right = ParagraphStyle("cellr", parent=cell_style, alignment=TA_RIGHT)

        header_row = ["Date", "Catégorie", "Fournisseur", "Montant", "Type"]
        tdata = [header_row]
        for r in rows:
            tdata.append([
                r["date_fmt"],
                Paragraph((r["categorie"] or "")[:60], cell_style),
                Paragraph((r["fournisseur"] or "")[:80], cell_style),
                Paragraph(f'{r["montant"]:.2f} €', cell_right),
                r["type"],
            ])
        tdata.append(["TOTAL", "", "", Paragraph(f'<b>{total_montant:.2f} €</b>', cell_right), ""])

        # Largeurs colonnes : 17cm utilisables (A4 21cm - 2x2cm marges)
        col_widths = [17*cm*0.15, 17*cm*0.20, 17*cm*0.25, 17*cm*0.20, 17*cm*0.20]
